...
public sealed class UnityAspectRuntime : IAspectRuntime
{
    ...
    public IDisposable EnterMethod(Type serviceType, string methodName, object?[] parameters)
    {
        ...
        if (_verboseLogging)
        {
            Debug.Log($"[PintoBean] → {serviceType.Name}.{methodName} (params: {parameters?.Length ?? 0})");
        }
    ...
    public void ExitMethod(IDisposable context, object? result)
    {
        if (context is UnityMethodContext methodContext)
        {
            ...
            Debug.Log($"[PintoBean] ✓ {methodContext.ServiceType.Name}.{methodContext.MethodName} completed in {duration:F2}ms → {resultType}");
    ...
    public void RecordException(IDisposable context, Exception exception)
    {
        ...
        {
            var duration = methodContext.GetDuration();
            Debug.LogError($"[PintoBean] ✗ {methodContext.ServiceType.Name}.{methodContext.MethodName} failed in {duration:F2}ms: {exception.GetType().Name} - {exception.Message}");
        }
        else
        {
            Debug.LogError($"[PintoBean] ✗ Exception: {exception.GetType().Name} - {exception.Message}");
        }
    }
    ...
    public void RecordMetric(string name, double value, params (string Key, object Value)[] tags)
    {
        ...
        Debug.Log($"[PintoBean] 📊 {name}: {value:F2}{tagString}");
    }
    ...
    public IDisposable StartOperation(string operationName, IReadOnlyDictionary<string, object>? metadata = null)
    {
        ...
        if (_verboseLogging)
        {
            ...
            Debug.Log($"[PintoBean] ⏰ Started: {operationName}{metadataString}");
        }
